...
Tab6:AddToggle({
	...
	Value = getgenv().Config.bossHop, -- ปรับค่าToggle true/false or Config
    Callback = function(x)
		getgenv().Config.bossHop = x
      _TweenCanCle(getgenv().Config.bossHop)
    end
})
			  ...
			  until not getgenv().Config.boss or v.Humanoid.Health <= 0
					  ...
					  if game:GetService("ReplicatedStorage"):FindFirstChild(SelectBoos) then
						...
						if not getgenv().Config.fastattack then
							...
							if getgenv().Config.bossHop then
								Hop()
							end
							if getgenv().Config.boss == false then
								UseFast = false
